...
namespace LinqExercises.Controllers
{
    public class ProductsController : ApiController
    {
        ...
        public IHttpActionResult GetDiscontinuedCount()
        {
            ...
            int productCount = _db.Products.Where(p => p.Discontinued == true).Count();
        ...
        public IHttpActionResult GetProductsInCategory(string categoryName)
        {
            ...
            var query = from products in _db.Products
                        join catgories in _db.Categories
                        on products.CategoryID equals catgories.CategoryID
                        where catgories.CategoryName.Contains(categoryName)
                        select products;
            if (query == null)
            {
                return NotFound();
        ...
        public IHttpActionResult GetStockReport()
        {
            ...
            var query = from products in _db.Products
                        ...
                        select new { Product = products, TotalStockUnits = products.UnitsInStock };
            if (query == null)
                return NotFound();
            else
                return Ok(query);
